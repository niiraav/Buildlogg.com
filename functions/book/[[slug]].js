...
function renderBookingPage(merchant, services, availableSlots) {
  ...
  }).join('') : `<div class="no-services">${merchant.business_type === 'beauty' ? '<p>Services coming soon.</p>' : `<p>${escapeHtml(bn)} hasn't set up their services yet.</p>`}${ph?`<p>Contact them directly:</p>${ph}`:''}</div>`;
