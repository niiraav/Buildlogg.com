...
function renderBookingPage(merchant, services, availableSlots) {
  ...
  const tl = merchant.specialty || (merchant.trade === 'other' ? (merchant.trade_other || '') : (merchant.trade || ''));
  ...
  }).join('') : `<div class="no-services">${(merchant.app_mode === 'bookings' || merchant.app_mode === 'both' || merchant.business_type === 'beauty') ? '<p>Services coming soon.</p>' : `<p>${escapeHtml(bn)} hasn't set up their services yet.</p>`}${ph?`<p>Contact them directly:</p>${ph}`:''}</div>`;
...
export async function onRequest(context) {
  ...
  if (request.method === 'GET') {
    try {
      ...
      for (const r of pendingRequests||[]) {
        ...
        // Use total_duration if available, otherwise default to 60 min
  ...
  if (request.method === 'POST') {
    try {
      ...
      if (reqMinutes < startMin || reqMinutes + slotDuration > endMin) return json({error:'That time is outside working hours'}, 409);
      ...
      if (merchant.booking_break_start && merchant.booking_break_end) {
        ...
        if (reqMinutes < bEnd && reqMinutes + slotDuration > bStart) return json({error:'That time is during a break period'}, 409);
      }
      const insertBody = {
        ...
        if (depositAmount >= 0.50) { // Stripe minimum charge
          ...
          if (STRIPE_KEY) {
            try {
              ...
              // Stripe Connect: route to merchant's account
