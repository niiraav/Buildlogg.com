...
export async function onRequest(context) {
  ...
  if (request.method === 'POST') {
    try {
      ...
      // BU-5: If merchant requires deposits and has Stripe connected, create a checkout session
      const bookingRequestId = result.id || (result[0] && result[0].id);
      if (merchant.payment_terms === 'deposit' && merchant.stripe_connected && merchant.deposit_pct > 0 && body.serviceAmount > 0) {
        const depositAmount = body.serviceAmount * (merchant.deposit_pct / 100);
