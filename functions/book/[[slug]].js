  ...
  for (let d = 0; d < daysAhead; d++) {
    ...
    for (let h = 9; h < 17; h++) {
      for (let m = 0; m < 60; m += serviceDurationMin) {
        ...
        if (!isBooked) { const t = `${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}`; daySlots.push(t); }
      }
    }
...
export async function onRequest(context) {
  ...
  if (request.method === 'GET') {
    try {
      ...
      const sd = (services&&services.length>0&&services[0].duration_minutes)||60;
      const bh = merchant.booking_buffer_hours||24;
      const slots = computeAvailableSlots(bookedSlots, sd, bh);
      if (slots.length === 0) return renderNoSlots(merchant);
      return html(renderBookingPage(merchant, services||[], slots));
  ...
  if (request.method === 'POST') {
    try {
      ...
      const rdt = `${body.requestedDate}T${body.requestedTime}:00`;
      ...
      const sS = new Date(rdt), sE = new Date(sS.getTime()+(body.serviceDuration||60)*60*1000);
